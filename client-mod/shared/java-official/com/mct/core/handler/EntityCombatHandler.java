...
public final class EntityCombatHandler extends ActionHandler {
    ...
    public EntityCombatHandler(Minecraft client, ClientStateTracker stateTracker, MovementHandler movementHandler, InputHandler inputHandler) {
    ...
    private Map<String, Object> listEntities(Map<String, Object> params) {
        ...
        for (Entity entity : clientWorld(player).entitiesForRendering()) {
    ...
    private Map<String, Object> entityInfo(Map<String, Object> params) {
        ...
        Entity entity = clientWorld(requirePlayer()).getEntity(id);
        if (entity == null) {
    ...
    private Map<String, Object> attackEntity(Map<String, Object> params) {
        ...
        requireInteractionManager().attack(player, entity);
    ...
    private Map<String, Object> interactEntity(Map<String, Object> params) {
        LocalPlayer player = requirePlayer();
        Entity entity = EntityHelper.findEntity(player, requireFilter(params));
        InteractionResult result = requireInteractionManager().interact(player, entity, new EntityHitResult(entity), InteractionHand.MAIN_HAND);
        return com.mct.core.util.MctMaps.mapOf("success", result.consumesAction(), "entityId", entity.getId(), "entityType", String.valueOf(McRegistries.entityTypeId(entity.getType())));
    }
    ...
    private Map<String, Object> mountEntity(Map<String, Object> params) {
        Map<String, Object> interaction = runOnClientThread(() -> {
            ...
            Entity entity = EntityHelper.findEntity(player, requireFilter(params));
            InteractionResult result = requireInteractionManager().interact(player, entity, new EntityHitResult(entity), InteractionHand.MAIN_HAND);
            return com.mct.core.util.MctMaps.mapOf("accepted", result.consumesAction(), "vehicleId", entity.getId());
        });
    ...
    private Map<String, Object> dismountEntity() {
        ...
        Boolean dismounted = pollUntil(2.0D, () -> !requirePlayer().isPassenger(), Boolean::booleanValue);
    ...
    private Map<String, Object> combatPickup(Map<String, Object> params) {
        ...
        while (Duration.between(startedAt, Instant.now()).toMillis() < (long) (timeoutSeconds * 1000.0D)) {
            ...
            pollUntil(pickupWaitTimeout, () -> clientWorld(requirePlayer()).getEntity(entityId) == null, Boolean::booleanValue);
        }
    ...
    private Map<String, Object> combatAttackLoop(Map<String, Object> filter, double timeoutSeconds, boolean approachTarget) {
        ...
        while (Duration.between(startedAt, Instant.now()).toMillis() < (long) (timeoutSeconds * 1000.0D)) {
            ...
            Boolean attacked = runOnClientThread(() -> {
                LocalPlayer player = requirePlayer();
                Entity entity = clientWorld(player).getEntity(asInt(target.get("entityId")));
    ...
    private Map<String, Object> nearestItemEntity(double radius) {
        ...
        return com.mct.core.util.MctMaps.mapOf(
            ...
            "y", nearest.getY(),
            "z", nearest.getZ(),
            "item", ClientDataHelper.itemToMap(nearest.getItem())
        );
    }
    ...
    private boolean targetDefeatedOnClientThread(@Nullable Integer entityId) {
        ...
        return !EntityHelper.isEntitySelectable(clientWorld(requirePlayer()).getEntity(entityId));
    }
}
