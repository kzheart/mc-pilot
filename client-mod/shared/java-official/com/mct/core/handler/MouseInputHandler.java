...
public final class MouseInputHandler extends ActionHandler {
    ...
    public MouseInputHandler(Minecraft client, ClientStateTracker stateTracker, InputHandler keyboardInput) {
    ...
    public void moveMouseTo(int scaledX, int scaledY) {
        runOnClientThread(() -> {
            ...
            ((MouseInputBridge) client.mouseHandler).mct$onCursorPos(window, rawX, rawY);
            return true;
        });
    }
    ...
    void dispatchMouseButton(String button, int action) {
        ...
        runOnClientThread(() -> {
            ((MouseInputBridge) client.mouseHandler).mct$onMouseButton(client.getWindow().handle(), glfwButton, action, 0);
            return true;
        });
    }
    ...
    private Map<String, Object> inputScroll(Map<String, Object> params) {
        ...
        moveMouseTo(x, y);
        runOnClientThread(() -> {
            ((MouseInputBridge) client.mouseHandler).mct$onMouseScroll(client.getWindow().handle(), 0.0D, delta);
            return true;
        });
        return com.mct.core.util.MctMaps.mapOf(
            "scrolled", true,
    ...
    private LinkedHashMap<String, Object> currentMousePosition() {
        LinkedHashMap<String, Object> result = new LinkedHashMap<>();
        result.put("x", rawToScaledX(client.mouseHandler.xpos()));
        result.put("y", rawToScaledY(client.mouseHandler.ypos()));
        return result;
    }
