...
public final class EntityHelper {
    ...
    public static Entity findEntity(LocalPlayer player, Map<String, Object> filter) {
        ...
        for (Entity entity : ClientVersionModulesHolder.get().clientWorld().getClientWorld(player).entitiesForRendering()) {
            if (entity == player) {
                continue;
            }
    ...
    public static int countEntities(LocalPlayer player, Map<String, Object> filter) {
        String type = filter.containsKey("type") ? normalizeEntityType(String.valueOf(filter.get("type"))) : null;
        Double maxDistance = filter.containsKey("maxDistance") ? asDouble(filter.get("maxDistance")) : null;
        int count = 0;
        for (Entity entity : ClientVersionModulesHolder.get().clientWorld().getClientWorld(player).entitiesForRendering()) {
