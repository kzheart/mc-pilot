...
export function createLookCommand() {
  const command = new Command("look").description(
    "Camera / view direction control",
  );
  ...
  command
    ...
    .requiredOption(
      "--yaw <yaw>",
      "Horizontal angle (-180 to 180: 0=south, -90=east, 90=west, ±180=north)",
      Number,
    )
    .requiredOption(
      "--pitch <pitch>",
      "Vertical angle (-90=up, 0=horizon, 90=down)",
      Number,
    )
    .action(
