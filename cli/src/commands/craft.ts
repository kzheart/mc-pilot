  ...
  if (
    ...
    if (slots.length !== 9) {
      throw new MctError(
        {
          code: "INVALID_PARAMS",
          message: "recipe.slots must contain exactly 9 entries",
        },
        4,
      );
    }
...
export function createCraftCommand() {
  return new Command("craft")
    ...
    .action(
      createRequestAction("craft.craft", ({ options }) => ({
        recipe: normalizeCraftRecipe(
          parseJson(String(options.recipe), "recipe"),
        ),
      })),
    );
}
...
export function createRecipeCommand() {
  ...
  command
    ...
    .requiredOption(
      "--recipe <json>",
      'Recipe JSON: 3 row arrays or {"slots":[...9 entries...]}',
    )
    .action(
      createRequestAction("craft.craft", ({ options }) => ({
        recipe: normalizeCraftRecipe(
...
export function createAnvilCommand() {
  return new Command("anvil")
    ...
    .requiredOption(
      "--input-slot <slot>",
      "Inventory slot of the item to rename (0-8: hotbar, 9-35: main)",
      Number,
    )
...
export function createEnchantCommand() {
  return new Command("enchant")
    ...
    .requiredOption(
      "--option <index>",
      "Enchantment option: 0 = top, 1 = middle, 2 = bottom",
      Number,
    )
    .action(
      createRequestAction("craft.enchant", ({ options }) => ({
...
export function createTradeCommand() {
  return new Command("trade")
    ...
    .action(
      createRequestAction("craft.trade", ({ options }) => ({
        index: options.index,
      })),
    );
}
