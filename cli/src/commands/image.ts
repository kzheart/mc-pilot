...
function parseRect(value: string | undefined, field: string): Rect | undefined {
  ...
  if (parts.length !== 4 || parts.some((part) => !Number.isFinite(part))) {
    throw new MctError(
      { code: "INVALID_PARAMS", message: `${field} must use x,y,w,h format.` },
      4,
    );
  }
...
function parseRgb(
  ...
  if (
    ...
    throw new MctError(
      {
        code: "INVALID_PARAMS",
        message: "--background must use r,g,b format.",
      },
      4,
    );
  }
...
function clampRect(rect: Rect | undefined, image: PNG): Rect {
  ...
  const right = Math.max(
    x,
    Math.min(image.width, Math.floor(raw.x + raw.width)),
  );
  const bottom = Math.max(
    y,
    Math.min(image.height, Math.floor(raw.y + raw.height)),
  );
  ...
  for (let y = region.y; y < region.y + region.height; y++) {
    for (let x = region.x; x < region.x + region.width; x++) {
      ...
      if (
        ...
        colorDistance(rgb, background) > options.threshold
      ) {
...
function collectTemplatePixels(
  template: PNG,
  alphaThreshold: number,
  maxSamples: number,
): Pixel[] {
    ...
    for (let x = 0; x < template.width; x++) {
      ...
      if (template.data[offset + 3] > alphaThreshold) {
        pixels.push({
          x,
          y,
          r: template.data[offset],
          g: template.data[offset + 1],
          b: template.data[offset + 2],
        });
      }
    }
  }
...
function scoreTemplateAt(
  ...
  for (const sample of samples) {
    ...
    score +=
      Math.abs(screenshot.data[offset] - sample.r) +
...
export function createImageCommand() {
  const command = new Command("image").description(
    "Image analysis helpers for screenshots and UI offset tuning",
  );
  ...
  command
    ...
    .option(
      "--background <rgb>",
      "Background color to ignore, format r,g,b. Defaults to top-left pixel.",
    )
    .option(
      "--threshold <value>",
  ...
  command
    ...
    .description(
      "Locate a transparent PNG template inside a screenshot, useful for tuning GUI title offsets",
    )
    ...
    .option(
      "--expected <rect>",
      "Expected rectangle x,y,w,h; output includes delta from it",
    )
    ...
    .option(
      "--stride <pixels>",
      "Search stride in screenshot pixels",
      (value) => Number(value),
      2,
    )
    .option(
      "--alpha-threshold <value>",
      "Template alpha threshold",
      (value) => Number(value),
      16,
    )
    .option(
      "--max-samples <count>",
      "Maximum template pixels to sample",
      (value) => Number(value),
      4000,
    )
    .action(
      wrapCommand(
        async (
          ...
          const screenshotPath = resolveProjectRelativePath(
            context,
            args[0] ?? "",
          );
          const templatePath = resolveProjectRelativePath(
            context,
            args[1] ?? "",
          );
          ...
          const region = clampRect(
            parseRect(options.region, "--region"),
            screenshot,
          );
          const expected = parseRect(options.expected, "--expected");
          const samples = collectTemplatePixels(
            template,
            ...
            throw new MctError(
              {
                code: "INVALID_PARAMS",
                message:
                  "Template has no visible pixels after alpha filtering.",
              },
              4,
            );
          }
            ...
            : Array.from(
                {
                  length: Math.max(
                    1,
                    Math.floor(
                      (Number(options.maxScale) - Number(options.minScale)) /
                        Number(options.scaleStep),
                    ) + 1,
                  ),
                },
                (_, index) =>
                  Number(
                    (
                      Number(options.minScale) +
                      index * Number(options.scaleStep)
                    ).toFixed(4),
                  ),
              );
          ...
          for (const scale of scales) {
            ...
            for (
              ...
              y += Number(options.stride)
            ) {
              for (
                let x = region.x - width;
                x <= region.x + region.width;
                x += Number(options.stride)
              ) {
          ...
          if (!best) {
            throw new MctError(
              {
                code: "INVALID_STATE",
                message: "No template match candidate found.",
              },
              3,
            );
          }
