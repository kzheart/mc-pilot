...
export function createInputCommand() {
  ...
  command
    ...
    .option(
      "--modifiers <modifiers>",
      "Modifier keys, comma-separated: shift,ctrl,alt",
    )
    .action(
  ...
  keyCommand
    ...
    .argument(
      "<key>",
      "Key name (e.g. w, a, s, d, space, escape, shift, enter, tab, e, f1)",
    )
    .action(
      createRequestAction("input.key-press", ({ args }) => ({
        key: String(args[0]),
      })),
    );
  ...
  keyCommand
    ...
    .action(
      createRequestAction(
        ...
        ({ options }, context) =>
          withTransportTimeoutBuffer(
            Math.max(Number(options.duration ?? 0) / 1000 + 2, 3),
  ...
  keyCommand
    ...
    .argument("<key>", "Key name")
    .action(
      createRequestAction("input.key-down", ({ args }) => ({
        key: String(args[0]),
      })),
    );
  ...
  keyCommand
    ...
    .argument("<key>", "Key name")
    .action(
      createRequestAction("input.key-up", ({ args }) => ({
        key: String(args[0]),
      })),
    );
  ...
  command
    ...
    .action(
      createRequestAction("input.type", ({ args }) => ({
        text: String(args[0]),
      })),
    );
