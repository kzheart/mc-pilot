...
const REQUEST_CASES: RequestCase[] = [
  ...
  },
  {
    ...
    argv: [
      "craft",
      "--recipe",
      '{"slots":["oak_planks",null,null,"oak_planks",null,null,null,null,null]}',
    ],
    action: "craft.craft",
    params: {
      recipe: [
...
function collectLeafCommands(
  ...
  return command.commands.flatMap((child) =>
    collectLeafCommands(child, [...parents, child.name()]),
  );
}
...
async function createRequestTestHarness(options?: {
  ...
  return {
    async runCli(args: string[]) {
      const { stdout } = await execFileAsync(
        process.execPath,
        [path.join(cwd, "dist/index.js"), ...args],
        {
          cwd: projectDir,
          env: {
            ...process.env,
...
test("CLI leaf commands are fully covered by automated tests", () => {
  ...
  const coveredLeafCommands = [
    ...NON_REQUEST_LEAF_COMMANDS,
    ...REQUEST_CASES.map((entry) => entry.leaf),
  ].sort();
  ...
  try {
    for (const entry of REQUEST_CASES) {
      await t.test(entry.leaf, async () => {
        ...
        assert.equal(
          (result.data as { data: { echoedAction: string } }).data.echoedAction,
          entry.action,
        );
        assert.deepEqual(
          (result.data as { data: { params: Record<string, unknown> } }).data
            .params,
          entry.params,
        );
      });
...
test("CLI request commands honor explicit timeout overrides", async () => {
  ...
  try {
    const chatWait = await harness.runCli([
      "chat",
      "wait",
      "--match",
      "Joined",
    ...
    assert.equal(
      (chatWait.data as { data: { echoedAction: string } }).data.echoedAction,
      "chat.wait",
    );
