...
export async function applyArm64LwjglPatch(
  ...
  for (let depth = 0; depth < 5; depth++) {
    ...
    if (
      (json.libraries || []).some((lib: { name?: string }) =>
        lib.name?.startsWith("org.lwjgl:lwjgl:3.2"),
      )
    ) {
      needsJsonPatch = true;
      break;
    ...
    if (
      (json.libraries || []).some((lib: { name?: string }) =>
        lib.name?.startsWith("org.lwjgl:lwjgl:3.3"),
      )
    ) {
      break;
    }
  ...
  console.error(
    "[MCT] Applying LWJGL 3.3.1 arm64 patch (based on HMCL NativePatcher)...",
  );

  const { open, walkEntriesGenerator, openEntryReadStream } = await import(
    "@xmcl/unzip"
  );
  ...
  for (const [libKey, entry] of Object.entries(LWJGL_ARM64_PATCH.natives)) {
    const localJar = await downloadJar(entry, minecraft, fetchImpl).catch(
      (e) => {
        console.error(`[MCT]   Skip ${libKey}: ${(e as Error).message}`);
        return null;
      },
    );
    if (!localJar) {
    ...
    for await (const zipEntry of walkEntriesGenerator(zip)) {
      ...
      await pipelineAsync(
        await openEntryReadStream(zip, zipEntry),
  ...
  for (const [libKey, entry] of Object.entries(LWJGL_ARM64_PATCH.jars)) {
    await downloadJar(entry, minecraft, fetchImpl).catch((e) =>
      console.error(`[MCT]   Skip ${libKey}: ${(e as Error).message}`),
    );
  }
  ...
  for (let depth = 0; depth < 5; depth++) {
    ...
    const json = JSON.parse(raw);
    const hasOldLwjgl = (json.libraries || []).some((lib: { name?: string }) =>
      lib.name?.startsWith("org.lwjgl:lwjgl:3.2"),
    );
    if (hasOldLwjgl) {
      const newLibraries = [];
      for (const lib of json.libraries) {
        ...
        if (
          !(baseKey in LWJGL_ARM64_PATCH.jars) &&
          !(baseKey in LWJGL_ARM64_PATCH.natives)
        ) {
        ...
        if (baseKey in LWJGL_ARM64_PATCH.jars && lib.downloads?.artifact) {
          const jarEntry =
            LWJGL_ARM64_PATCH.jars[
              baseKey as keyof typeof LWJGL_ARM64_PATCH.jars
            ];
          const newName =
            baseKey === "org.lwjgl:lwjgl-glfw"
        ...
        if (
          ...
          const nativeEntry =
            LWJGL_ARM64_PATCH.natives[
              baseKey as keyof typeof LWJGL_ARM64_PATCH.natives
            ];
          newLib.downloads = {
            ...newLib.downloads,
            classifiers: {
