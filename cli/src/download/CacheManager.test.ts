...
test("CacheManager builds deterministic cache paths", () => {
  ...
  assert.equal(
    cache.getLoaderDir("fabric"),
    path.join("/tmp/mct-cache", "client", "fabric"),
  );
  assert.equal(
    cache.getModFile("mct-client-mod-1.20.4-fabric.jar"),
    path.join("/tmp/mct-cache", "mod", "mct-client-mod-1.20.4-fabric.jar"),
  );
});
