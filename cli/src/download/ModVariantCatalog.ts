...
const DEFAULT_VARIANTS_PATH = fileURLToPath(
  new URL("../../data/variants.json", import.meta.url),
);
...
export async function loadModVariantCatalog(
  filePath = DEFAULT_VARIANTS_PATH,
): Promise<ModVariantCatalog> {
  const raw = await readFile(filePath, "utf8");
  return JSON.parse(raw) as ModVariantCatalog;
}

export function loadModVariantCatalogSync(
  filePath = DEFAULT_VARIANTS_PATH,
): ModVariantCatalog {
...
export function sortVariants(variants: ModVariant[]) {
  return [...variants].sort((left, right) => {
    const versionDelta = compareVersions(
...
export function findVariantByVersionAndLoader(
  catalog: ModVariantCatalog,
  minecraftVersion: string,
  loader: ModVariant["loader"],
) {
  return catalog.variants.find(
    (variant) =>
      variant.minecraftVersion === minecraftVersion &&
...
export function getSupportedMinecraftVersions(catalog: ModVariantCatalog) {
  return [
    ...new Set(catalog.variants.map((variant) => variant.minecraftVersion)),
  ]
